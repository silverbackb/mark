...
import { z } from "zod";
import { migrate, insertEvent, listSlugs, summary, funnel, compare, friction, journey, breakdown, purge, purgeOldEvents, registerSnippet, resolveUrl, resolveByUrl, resolveBySlug, insertUnresolvedEvent, listSnippets, recentEvents, LIMITS, } from "./db.js";
...
// --- HTTP tracker script ---
function trackerScript(slug, wid, projectId) {
...
Use when: you want to replay or debug a specific user's path. Complement with mark_funnel for aggregate view.`,
    ...
    server.registerTool("mark_breakdown", {
...
Complement with mark_funnel to measure conversion from a specific page.`,
        ...
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ slug, event_name, property, days, tag, limit }) => ok(await breakdown(MCP_WORKSPACE_ID, slug, event_name, property, days ?? 30, tag, limit ?? 30)));
