...
function registerMcp(configPath: string, serverBinPath: string): boolean {
  if (!existsSync(configPath)) return false;
  try {
    const config = JSON.parse(readFileSync(configPath, "utf-8"));
    if (!config.mcpServers) config.mcpServers = {};
    if (config.mcpServers[MCP_NAME]) return false; // already registered
...
for (const t of skillTargets) {
  if (installSkill(t.dir)) skillInstalled.push(t.name);
}

const mcpRegistered: string[] = [];
for (const t of mcpTargets) {
  if (registerMcp(t.config, serverBin)) mcpRegistered.push(t.name);
}

console.log(`\n@silverbackbase/mark v${PKG_VERSION} — init\n`);
if (skillInstalled.length) skillInstalled.forEach(n => console.log(`  skill installed — ${n}`));
else console.log("  skill already installed or target not found");
if (mcpRegistered.length) mcpRegistered.forEach(n => console.log(`  MCP registered — ${n}`));
else console.log("  MCP already registered or config not found");
